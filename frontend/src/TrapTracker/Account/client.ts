...
export const get_users = async () => {
    const response = await apiClient.get(`${USERS_API}`);
    console.log("response.data", response.data.users);
    return response.data;
};

export const getProfile = async (id: any) => {
    const response = await apiClient.post(`${USERS_API}/profile`, { id: id });
...
export const signin = async (credentials: any) => {
    ...
    const response = await apiClient.post(`${USERS_API}/signin`, credentials);
...
export const updateUser = async (user: any) => {
    const response = await apiClient.put(`${USERS_API}/update`, user);
...
export const updateProfile = async (profile: any, id: any) => {
    ...
    const response = await apiClient.put(`${USERS_API}/profile/update`, { profile: profile, id: id });
    console.log("response.data", response.data);
    const profile_array = response.data.profile[0];
    const profile_json = {
        firstName: profile_array[1],
